...
var logFile = fs.createWriteStream('data/marki_modeli_kuzova_gody_modific.csv', { flags: 'w+' });
var logger = function () {
  logFile.write(util.format.apply(null, arguments) + '\n');
};

var stream = fs.createReadStream("data/marki_modeli_kuzova_gody.csv");
...
var getItems = function (item, callback) {
  ...
  if (!item.key) {
    var tmp = {};
    tmp[item.modific] = item.link;
    _RES[item.id] = item;
    _RES[item.id].items = tmp;
    callback();
  } else {
    request
      .get(url+item.key)
